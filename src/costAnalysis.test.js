...
const firstOrSecondComplexity = 3
...
describe('Cost analysis Tests', () => {
  ...
  test('should consider recursive cost computation when an inline fragment is used', () => {
    const limit = 10
    const ast = parse(`
      query {
        first(limit: ${limit}) {
  ...
  test('if complexity is provided as a function in costMap, we compute the score with it', () => {
    const limit = 20
    const secondComplexity = 13
    const ast = parse(`
      query {
        first(limit: ${limit}) {
          second(limit: ${limit})
    ...
    const costMap = {
      ...
      First: {
        second: {
          complexity: ({ _fieldArgs }) => secondComplexity
        }
      }
    }
    ...
    visit(ast, visitWithTypeInfo(typeInfo, visitor))
    const firstCost = limit * costMap.Query.first.complexity
    const secondCost = limit * secondComplexity
    const expectedCost = firstCost + secondCost
    expect(visitor.cost).toEqual(expectedCost)
  })
